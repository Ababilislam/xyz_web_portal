...
def save_order():
    ...
    if save_btn:
        if request.vars:
            ...
            if head_flag==True:   
                status="Draft"
                insert= f""" 
                insert into sm_order_head (cid,sl,client_name,mobile_no,level3_name,status,created_on,created_by) 
                values ('{c_id}','{last_order_sl}','{client_name}','{mobile}','{address}','{status}','{order_datetime}','{user_name}')
                """
                # return insert
    ...
    if delete_btn:
        ...
        if len(check_rec_exist)==0:
            ...
            update_query=f"delete from sm_order_temp  where id='{record_id}'"
            db.executesql(update_query)
    ...
    client_name_d = order_head[0]['client_name']
    mobile_d = order_head[0]['mobile_no']
    address_d = order_head[0]['level3_name']
    # return client_name, mobile,address
    
    return dict(
        item_data=item_data,page=0,temp_data=temp_data,last_order_sl=last_order_sl,
        client_name=client_name_d,mobile=mobile_d,address=address_d)
